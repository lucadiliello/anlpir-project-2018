...
class Loader:
    ...
    def load(self):
        ...
        if self.pkg_name == 'InsuranceQA':
            ...
            import jsonlines
            with jsonlines.open('data/TrecQA/valid.jsonl') as reader:
                for obj in reader:
                    obj['question'] = str(obj['question'])
                    obj['candidates'] = [{'sentence': str(cand['sentence']), 'label': cand['label']} for cand in obj['candidates']]
                    valid.append(obj)

            with jsonlines.open('data/TrecQA/train.jsonl') as reader:
                for obj in reader:
                    obj['question'] = str(obj['question'])
                    obj['candidates'] = [{'sentence': str(cand['sentence']), 'label': cand['label']} for cand in obj['candidates']]
                    train.append(obj)

            with jsonlines.open('data/TrecQA/test.jsonl') as reader:
                for obj in reader:
                    obj['question'] = str(obj['question'])
                    obj['candidates'] = [{'sentence': str(cand['sentence']), 'label': cand['label']} for cand in obj['candidates']]
                    test.append(obj)
